...
// Configurable endpoints for your own Google Form + published CSV.
// You can also define window.WC2026_CONFIG in index.html before app.js.
const APP_CONFIG = globalThis.WC2026_CONFIG || {};
const LEADERBOARD_CSV_URL = APP_CONFIG.leaderboardCsvUrl || '';
...
async function loadLeaderboard() {
  ...
  if (!LEADERBOARD_CSV_URL) {
    if (container) {
      container.innerHTML = '<p class="note-text" style="margin-top:20px;">Configura <strong>leaderboardCsvUrl</strong> en <strong>window.WC2026_CONFIG</strong> para mostrar el ranking.</p>';
    }
    return;
  }
  ...
  try {
    ...
    if (container) {
      container.innerHTML = '<p class="note-text" style="margin-top:20px;">No se pudo cargar el ranking ahora mismo. Revisa la URL del CSV y los permisos de publicación.</p>';
    }
    return;
  }
  ...
  rows.slice(1).forEach(row => {
    const prediction = extractPredictionFromCsvRow(row);
...
function extractPredictionFromCsvRow(row) {
  ...
  for (const cell of row) {
    ...
    try {
      ...
      // Minimal shape validation to avoid false positives on other JSON cells.
      if (parsed && (parsed.groups || parsed.knockout || parsed.awards)) {
        return parsed;
      }
    } catch (e) {
      console.debug('Skipping non-prediction JSON cell in leaderboard CSV.', e);
    }
  }
...
function renderLeaderboardList(submissions) {
  ...
  if (!submissions.length) {
    ...
    empty.textContent = 'Todavia no hay apuestas registradas.';
    list.appendChild(empty);
    return;
  }
...
function isSubmissionClosed() {
  if (!SUBMISSION_DEADLINE_ISO) return false;
  const deadline = new Date(SUBMISSION_DEADLINE_ISO);
...
async function confirmSubmitPrediction() {
  ...
  if (!FORM_ID || !ENTRY_ID) {
    showToast('Falta configurar formId y entryId para poder enviar la quiniela.', true);
    return;
  }

  if (isSubmissionClosed()) {
    showToast('La quiniela esta cerrada. Ya no se aceptan nuevas apuestas.', true);
    return;
  }
  ...
  try {
    ...
    hideLoading();
    fireConfetti();
    showToast('¡Apuesta registrada! Tarda unos segundos en asomar por el ranking. Mucha suerte, crack de SPERMANS.');
  } catch(e) {
    hideLoading();
...
async function init() {
  ...
  if (isSubmissionClosed()) {
    ...
    if (submitBtn) {
      ...
      submitBtn.textContent = 'Quiniela cerrada';
      submitBtn.title = 'La fecha limite de envio ya ha pasado';
    }
  }
}
